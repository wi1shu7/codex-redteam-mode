...
def decide_loop_action(
    *,
    ...
    # Highest priority: skill_card exit gate (scope-not-instruct pattern).
    # When a skill_card is active, loop stays within the skill domain until
    # evidence satisfies the exit gate or negative-exit criteria are met.
    # Exception: if the orchestrator gate has already passed (gate_ok + verify_passed),
    # it means we're transitioning across phases — the skill_card yields.
    if skill_card is not None and not (gate_ok and verify_passed):
        _artifacts = getattr(state, "evidence_artifacts", [])
        _iteration = getattr(state, "loop_iteration", 0)
        satisfied, gate_reason = skill_card.check_exit_gate(_artifacts, _iteration)
        _display = getattr(skill_card, "display_name", "skill")
        ...
        if satisfied:
            return _decision(
                ...
                next_step=f"Exit [{_display}] — exit gate satisfied ({gate_reason}). Proceed to reporting.",
                trigger="exit_gate_satisfied",
                feedback_gate="Skill Exit Gate",
                exit_condition="skill_domain_complete",
            )

        # Still in skill domain — check for pseudo-complete abuse
        if getattr(state, "pseudo_complete_count", 0) >= 2:
            _missing = gate_reason.replace("missing_artifacts:", "")
            return _decision(
                action="blocked",
                reason="repeated_pseudo_complete_in_skill",
                next_stage=stage,
                next_step=f"在 [{_display}] 域内停止声明完成。需提供真实证据: {_missing}",
                trigger="pseudo_complete_threshold",
                feedback_gate="Skill Exit Gate",
                exit_condition="block_until_evidence",
                required_artifact=_missing,
            )

        # Normal: not yet satisfied, keep working in the skill domain
        _missing = gate_reason.replace("missing_artifacts:", "")
        return _decision(
            action="verify",
            reason="exit_gate_pending",
            next_stage=stage,
            next_step=f"在 [{_display}] 域内继续收集证据。尚缺: {_missing}",
            trigger="exit_gate_pending",
            feedback_gate="Skill Exit Gate",
            exit_condition="collect_required_artifacts",
            required_artifact=_missing,
        )

    # Original logic (no skill_card active) follows below.
