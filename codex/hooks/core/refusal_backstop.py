...
def _transcript_file(transcript_path: str | None) -> Path | None:
    if not transcript_path:
        return None
    # Codex documents transcript_path but not a stable transcript wire format.
    # Transcript scanning/patching stays behind explicit backstop environment
    # opt-in; the normal turn path uses Stop.last_assistant_message instead.
